...
当前 24 条数据只能做 smoke test。对外发布前，应扩展到 800～1500 条人工复核数据，并把
测试集按事件/模板隔离。
...
|---|---:|---:|---:|---:|
| 原始模型 + 固定提示 | 待实测 | 待实测 | 待实测 | 待实测 |
| 原始模型 + Few-shot | 待实测 | 待实测 | 待实测 | 待实测 |
| LoRA 模型 | 待实测 | 待实测 | 待实测 | 待实测 |
...
- [ ] GitHub Actions 通过；
- [ ] 数据来源、生成过程、许可证和去隐私方法已写清；
- [ ] ModelScope 发布 LoRA Adapter 和模型卡；
- [ ] README 填入真实评测数字和失败案例；
- [ ] 在线 Demo 或演示视频可以打开；
- [ ] 不提交 Token、私人聊天、简历、手机号和内部数据；
